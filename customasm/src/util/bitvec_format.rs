...
//            result.push_str(&format!("{:1$x} | ", span.addr, addr_width));
            result.push_str(&format!(" {:0>1$x}", span.addr, addr_width));
...
//            result.push_str(&format!(" {:1$}", contents_str, content_width));      //comment this out if all you want are address and source assembly
            if digit_bits == 4 {result.push_str(&format!(" {:1$}", contents_str, content_width));}      
