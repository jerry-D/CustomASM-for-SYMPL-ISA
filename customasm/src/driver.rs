...
//		Some("annotated")    => OutputFormat::AnnotatedHex,  
		Some("annotated")    => OutputFormat::AnnotatedBin,  //AnnotatedBin now only provides {address sourceList} for uploading to debugger (no binary field)
		Some("annotatedhex") => OutputFormat::AnnotatedHex,  //AnnotatedHex has been modified so there are no spaces between bytes in hex code field
//		Some("annotatedbin") => OutputFormat::AnnotatedBin,
		...
		Some("hexdump")   => OutputFormat::HexDump,          //hexdump has been modified to produce a 64-bit wide hex file for use with Verilog $readmemh system task
...
//    opts.optopt("f", "format", "The format of the output file. Possible formats: binary, annotated, annotatedbin, binstr, hexstr, bindump, hexdump, mif, intelhex, deccomma, hexcomma, decc, hexc, logisim8, logisim16", "FORMAT");
    opts.optopt("f", "format", "The format of the output file. Possible formats: binary, annotated, annotatedhex, binstr, hexstr, bindump, hexdump, mif, intelhex, deccomma, hexcomma, decc, hexc, logisim8, logisim16", "FORMAT");
...
//	println!("{}", opts.usage(&format!("Usage: {} [options] <asm-file-1> ... <asm-file-N>", env!("CARGO_PKG_NAME"))));
	println!("{}", opts.usage(&format!("Usage: {} [options] <asm-file-1> ... <asm-file-N>", "customasm")));
}
...
//	println!("https://github.com/hlorenzi/customasm");
	println!("Original unmodified --> https://github.com/hlorenzi/customasm");
	println!("Modified for SYMPL Compute Engines --> https://github.com/jerry-d");
}
